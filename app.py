...
def init_db():
    """Initialize DB with data if not exists"""
    if not os.path.exists(DB_PATH):
        import sys
        sys.path.append(os.path.join(os.path.dirname(__file__), ".."))
        from setup_sqlite import create_tables, load_data, get_connection as gc
        conn = gc()
        create_tables(conn)
        load_data(conn)
        conn.close()
...
if menu == "🏠 Dashboard":
    ...
    with col8:
        ...
        fig4 = px.bar(df4, x="meal_type", y="count", color="meal_type", title="Listings by Meal Type")
    ...
    if not df.empty:
        ...
        with col5:
            fig = px.bar(df.groupby("location")["quantity"].sum().reset_index(),
                         x="location", y="quantity", color="quantity",
                         color_continuous_scale="Greens", title="Quantity by City")
            st.plotly_chart(fig, use_container_width=True)
        with col6:
    ...
    with st.form("add_form"):
        ...
        if submitted:
            if food_name and location:
                ...
                execute_dml("""INSERT INTO food_listings
                    (food_name, quantity, expiry_date, provider_id, provider_type,
                     location, food_type, meal_type)
                    VALUES (?,?,?,?,?,?,?,?)""",
                    (food_name, quantity, str(expiry_date), pid,
...
# ══════════════════════════════════════════════════════════════
    ...
    df = run_query("SELECT food_id, food_name, quantity, expiry_date, food_type, meal_type FROM food_listings ORDER BY food_id")
    ...
    with st.form("update_form"):
        ...
        new_ftype = col2.selectbox("Food Type", ["Vegetarian","Non-Vegetarian","Vegan"],
                                   index=["Vegetarian","Non-Vegetarian","Vegan"].index(row["food_type"]))
        new_mtype = col1.selectbox("Meal Type", ["Breakfast","Lunch","Dinner","Snacks"],
                                   index=["Breakfast","Lunch","Dinner","Snacks"].index(row["meal_type"]))

        submitted = st.form_submit_button("✅ Update")
        if submitted:
            execute_dml("UPDATE food_listings SET quantity=?, food_type=?, meal_type=? WHERE food_id=?",
                        (new_qty, new_ftype, new_mtype, fid))
...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown("---")
    st.warning("⚠️ This will permanently delete the listing and its related claims.")
...
# ══════════════════════════════════════════════════════════════
    ...
    with tab1:
        st.dataframe(run_query("SELECT * FROM providers"), use_container_width=True)
    with tab2:
        st.dataframe(run_query("SELECT * FROM receivers"), use_container_width=True)
    with tab3:
        st.dataframe(run_query("SELECT * FROM food_listings"), use_container_width=True)
    with tab4:
        st.dataframe(run_query("SELECT * FROM claims"), use_container_width=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    with tab1:
        ...
        if city_filter != "All": q += f" WHERE city='{city_filter}'"
    ...
    with tab2:
        ...
        if city_filter2 != "All": q2 += f" WHERE city='{city_filter2}'"
